...
void generate_numbers()
{
  ...
  uint64_t * memptr = (uint64_t*)vaddr2;
...
{  
  uint64_t *memptr1;
  uint64_t *memptr2;
  unsigned long packet_length=16;

  //select app mux 1 for application in the firmware. 
  int max_tlp = wupperCard.dma_max_tlp_bytes();
  //wupperCard.cfg_set_option("INTERLAKEN_PACKET_LENGTH",packet_length);
  generate_numbers();
  
  printf("Reading data from buffer 1...\n");
  //packet_length = wupperCard.cfg_get_option("INTERLAKEN_PACKET_LENGTH");
  printf("Packet lenght is set to: %lu \n", packet_length);
  wupperCard.dma_to_host(0, paddr1, (packet_length*8+1)*256, max_tlp, 0);
  wupperCard.dma_from_host(1, paddr2, packet_length*8*512, max_tlp, 0);
...
main(int argc, char** argv)
{
  ...
  while ((opt = getopt(argc, argv,"brfdh")) != -1) {
    switch (opt) {
