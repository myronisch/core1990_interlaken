...
/***************************************************************************/
{
  ...
  if (RCC_ERROR_MAJOR(err)) 
  {
    ...
    else 
    {
      flag = 1;
      strcpy(Maj_Pid_str, "");
      strcpy(Maj_en_str, "<rcc_error_init not called. Maybe you have called a function of another library without opening it.>");
    }
  }
  ...
  if (RCC_ERROR_MINOR_ERRNO(err)) 
  {
    ...
    else 
    {
      flag = 1;
      strcpy(Min_Pid_str, "");
      strcpy(Min_en_str, "<rcc_error_init not called Maybe you have called a function of another library without opening it.>");
    }
  }
...
/***********************************************************************/ 
{
  ...
  switch (err) 
  {
  case ERCC_OK:         strcpy(en_str, ERCC_OK_STR);
                        break;
  case ERCC_NOTOPEN:    strcpy(en_str, ERCC_NOTOPEN_STR);
                        break;
  case ERCC_NOINIT:     strcpy(en_str, ERCC_NOINIT_STR);
                        break;
  case ERCC_STREAM:     strcpy(en_str, ERCC_STREAM_STR);
                        break;
  case ERCC_NOCODE:     strcpy(en_str, ERCC_NOCODE_STR);
                        break;
  default:              strcpy(en_str, ERCC_NOCODE_STR);
                        return(RCC_ERROR_RETURN(0, ERCC_NOCODE));
                        break; 
  }
  return( RCC_ERROR_RETURN(0,ERCC_OK) );
}
